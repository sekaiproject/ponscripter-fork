...
static struct FuncLUT{
    ...
    {"h_ligate", &ONScripterLabel::haeleth_ligature_controlCommand},
...
void ONScripterLabel::openAudio()
{
    ...
    if (Mix_OpenAudio(sample_rate, MIX_DEFAULT_FORMAT, MIX_DEFAULT_CHANNELS, DEFAULT_AUDIOBUF ) < 0) {
...
//--------LINE BREAKING ROUTINE-------------------------------------------------
	...
	while (1) {
		...
		bool ok = true;
		for (int offs = 1; ok && offs <= 6; ++offs) ok &= isxdigit(it[offs]);
		if (ok) {
		    it += 7;
		    continue;
		}
