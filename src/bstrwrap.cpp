...
void CBString::add (const char * s, int len) {
    if (BSTR_ERR == bcatblk(this, s, len)) {
	bstringThrow ("Failure in concatenate");
    }
}
...
int CBString::cmp (const CBString& b) const {
int ret;
	if (SHRT_MIN == (ret = bstrcmp ((bstring) this, (bstring) &b))) {
		bstringThrow ("CBString::cmp Unable to compare");
	}
	return ret;
}
...
CBString& CBString::ltrim (const CBString& b) {
	int l = nfindchr (b, 0);
...
CBString& CBString::rtrim (const CBString& b) {
...
CBString CBStringList::join () const {
int c;
size_t i;
CBString rv;
...
CBString CBStringList::join (const CBString& sep) const {
int c, sl = sep.length();
size_t i;
CBString rv;
...
CBString CBStringList::join (char sep) const {
int c;
size_t i;
CBString rv;
...
CBString CBStringList::join (unsigned char sep) const {
int c;
size_t i;
CBString rv;
...
CBStringList CBString::split (unsigned char splitChar, int n) const {
int p, i;
CBStringList rv;
	...
	do {
		...
		if (i >= p) {
		    rv.push_back (CBString (&(data[p]), i - p));
		    if (!--n && i + 1 <= length()) {
			rv.push_back (CBString ((const char*) &(data[i + 1])));
			return rv;
		    }
		}
		p = i + 1;
...
CBStringList CBString::split (const CBString& s, int n) const {
...
CBStringList CBString::splitstr (const CBString& s) const {
int p, i;
	...
	if (s.length() == 0) {
		for (i=0; i < length (); i++) {
			rv.push_back (CBString (data[i]));
		}
	} else {
		for (p=0; (i = find (s, p)) >= 0; p = i + s.length ()) {
			rv.push_back (midstr (p, i - p));
		}
		if (p <= length ()) {
			rv.push_back (midstr (p, length () - p));
		}
	}
	return rv;
}
...
CBStringList CBStream::split (const CBString& s) {
    CBStringList rv;
	if (0 > bssplitscb (m_s, (bstring) &s, streamSplitCb, 
	                    (void *) &rv)) {
...
CBStringList CBStream::split (unsigned char splitChar) {
CBString sc (splitChar);
CBStringList rv;
	if (0 > bssplitscb (m_s, (bstring) &sc,
	                    streamSplitCb, (void *) &rv)) {
		bstringThrow ("Split bstream failure");
	}
	return rv;
}

CBStringList CBStream::splitstr (const CBString& s) {
    CBStringList rv;
	if (0 > bssplitstrcb (m_s, (bstring) &s, streamSplitCb, 
	                    (void *) &rv)) {
		bstringThrow ("Split bstream failure");
	}
	return rv;
