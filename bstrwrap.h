/* -*- c++ -*-
 * This source file is part of the bstring string library.  This code
 * was written by Paul Hsieh in 2002-2007, and is covered by the BSD
 * open source license. Refer to the accompanying documentation for
 * details on usage and license.
 */

/*
 * This version has been modified somewhat by Haeleth, which was
 * probably a bad idea but seemed like a good one at the time; the
 * default interface was just a tad too inconvenient.
 *
 * Non-obvious changes to note are:
 * 
 * - It's no longer an error to initialise a string from NULL.  That's
 *   just treated as an empty string instead.
 *
 * - It's no longer a bounds error to read the null terminator.
 *   However, a warning is still printed, as it's probably something
...
//	CBString (int len, const char *s);   Has very unexpected behaviour!
...
// +haeleth	
    inline operator bool () const { return slen; }
    char* mutable_data () { return (char*) data; }

    // Append functions with length constraints.  It's assumed that
...
// -haeleth	    
	...
	// Search methods.
	int cmp (const CBString& b) const;
	...
	CBString& ltrim (const CBString& b = CBString (bsStaticBlkParms (" \t\v\f\r\n")));
	CBString& rtrim (const CBString& b = CBString (bsStaticBlkParms (" \t\v\f\r\n")));
	inline CBString& trim (const CBString& b = CBString (bsStaticBlkParms (" \t\v\f\r\n"))) {
		rtrim (b);
		ltrim (b);
		return *this;
	}
    ...
    CBStringList split (unsigned char splitChar, int n = 0) const;
    CBStringList split (const CBString& s, int n = 0) const;
    CBStringList splitstr (const CBString& s) const;
#endif
...
public:
    ...
    CBStringList split (unsigned char splitChar);
    CBStringList split (const CBString& s);
    CBStringList splitstr (const CBString& s);
...
struct CBStringList : public std::vector<CBString> {
    CBString join () const;
    CBString join (const CBString& sep) const;
    CBString join (char sep) const;
    CBString join (unsigned char sep) const;
};
#endif
